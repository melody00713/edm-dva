...
class PowerModal extends PureComponent {
  ...
  calcDelayTimer = () => {
    let delayTimer = this.state.durationtime;
  ...
  render() {
    ...
    const { delayTimer, activeOperation, durationtime } = this.state;
    return (
      <div className={`${styles.powerModal + (powerVisible === true ? 'active': '')}`}>
        <div className={styles.powerModalMask}>
          <Row type="flex" justify="space-around" align="middle" style={{ height: '100%', width: '60%', margin: '0 auto' }}>
            <Col span={4}>
              <CircleProgress className={styles.circle} onClick={this.operationPower.bind(null, 2)} active={activeOperation === 2} delay={powerType === 2} delaytime={delayTimer} durationtime={durationtime} name={'重启'} />
            </Col>
            <Col span={4}>
              <CircleProgress className={styles.circle} onClick={this.operationPower.bind(null, 1)} active={activeOperation === 1} delay={powerType === 1} delaytime={delayTimer} durationtime={durationtime} name={'关机'} />
            </Col>
            <Col span={4}>
              <CircleProgress className={styles.circle} onClick={this.hideModalHandler} active={activeOperation === 0} name={'取消'} />
            </Col>
          </Row>
        </div>
      </div>
